...
> version: 1.7 | last_updated: 2026-05-05 | author: Claude (Opus 4.7) | scope: 短期客戶 template 全域
...
mistake → soft lesson L-0012  ←─┐
                                  │ 同時、平行
CLAUDE.md 禁令 #8 + (territory-lint 已退役)  ←─┘
```
...
| F. 邊界遵守 | F3 | **否**（純規則 + CI gate）| 不是 skill、(territory-lint 已退役) |
...
| (territory-lint 已退役) CI（禁令 #8） | 4.24 才硬化、解決 PR#260/#261 真實傷口、改了會退化 |
