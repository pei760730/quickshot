...
| # | 路徑 | 內容 | 載入時機 |
|---|------|------|---------|
| 8 | `01-data-brain/transcripts/` | Kai 的語音筆記 / 文字原文完整保留 | 沉澱分析時才讀（每累積 2 篇新原文觸發，短期適配） |
...
- `transcripts/` 累積 ≥ 2 篇新原文 → 批次交叉沉澱分析（見下方）

### 原文沉澱機制

**觸發**：每累積 2 篇新原文（從上次沉澱後計算；短期客戶適配，原 KaiOS ≥5 為持續經營門檻、冷啟動匯入存量不計入。見 `.claude/rules/workflow.md` §對話開頭）
