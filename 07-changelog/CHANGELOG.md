...
- `workflow.md` v2.36 新增 §腳本版本對比 + 常用指令 `對比版本：VID-NNN` / `對比：[主題]`
- 判斷依據：語意差異摘要需要 AI 判斷（機械 diff 無價值）、repo 現況 0 支腳本、寫 feature code 違反禁令 #9「預測 → 不做」；資料層已齊（檔名 `_V*.md` 約定 + pipeline `script_path`）、無需新 CLI
- ROADMAP 該項移 ✅、標註升級條件（真實使用中對話層不夠 → 準則 F 退場測試評估升 CLI）

**Mode W metric（上線日記錄、下一個客戶結束時檢查）**：
- `對比版本` 觸發次數 ≥ 1（無客戶期間凍結、時鐘從下一個 `/init` 起算）
- 0 = 需求是預測出來的、ROADMAP 該項目直接刪除、不升 CLI
