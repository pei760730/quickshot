...
> 2. 對每候選跑 `title-rules.md` 核心原則 4 必含逐項評分（具體金額 / 反差動作 / 第一人稱「我」/ 留缺口或問號）
...
## Output Contract（「縮」、無 generation_trace）

Generation Skill 跑完輸出含 3 階段：**Bash tool 執行 save**（無 --trace）→ AI 自評 4 句（人話層、Kai 主視覺）→ 技術摘要（次要、Kai 要才展開）。任一缺 = task 沒完成。

> **「縮」核心**：v1.1-v1.3 圍繞 `generation_trace` 寫入強化（CLI 強制 / 人話雙層 / 動詞硬化）、但 trace 上線 30 天零消費（無任何 code 讀它做決策、見 workflow.md §Mode W）。短期客戶（≤30 天驗證型）不需要自我進化 forensic、整套 trace 機器已移除。§Output Contract 不再要求輸出 generation_trace fenced JSON、也不帶 `--trace` arg。**保留真實在運作的迴路**：save 寫偏好欄位 + verifier_prediction、品質回饋走 verifier_scores（見 Quality Skill）。
...
**為什麼這條最硬**：
- 「呼叫 CLI」是模糊動詞、Claude 對話中經常退化為「印 Bash code block 展示給 Kai」（4.6 慣性「Claude = 顧問、不是執行者」）。動詞強制為「**Bash tool 執行**」、無「印命令給 Kai 抄」中間層。
- 違反 = task 沒完成、回頭重做。
...
| 📊 預測 | `adoption-stats` 同 hook_type filter | 用近 30 支同類數字、不套話 |
...
→ 違反 §1。「呼叫 CLI」在 Claude 4.6 慣性下退化為「展示命令給 Kai」、強制為「Bash tool 執行」。

❌ **跳過人話 4 句、直接印 CLI 結果**：

> ✓ 存檔 VID-NNN（mode=dual-track / hook=B1）

→ 違反 §2、Kai 看完腳本要自己判斷可不可拍、回到「Claude 印機器訊息給 Kai」舊模式。
...
| Distillation | task 末累積 lesson evidence |
...
- **v1.4 內容更新**（2026-06-14、engine「縮」、不 bump 版本以免 stub frontmatter 不同步）：移除整套 generation_trace 輸出契約。trace 上線 30 天零消費（無任何 code 讀它做決策、v1.1-v1.3 三輪採用強化都在優化一個沒人讀的欄位、屬 workflow.md §Mode W「規則上線 0 metric = 未上線」的典型）。短期客戶不需要自我進化 forensic。§Output Contract 簡化為 Bash save（無 --trace）→ AI 自評 4 句 → 技術摘要；移除 `_meta.trace_required_statuses` 強制、set-trace / save-with-trace-from-stdin CLI、validate_generation_trace 機器。保留 verifier_scores 品質迴路不動。
