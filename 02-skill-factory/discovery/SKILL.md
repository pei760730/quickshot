...
## 選題第一性原則（依 `shared-references/direction-principles.md` 心法 A）

選題前先答：**「這支戳中觀眾哪一個還沒說出口的恐懼／驅動？」**——不是「我想拍什麼」、是「觀眾現在在想什麼」。答不出這題、題就還沒選對、信心分數一律不給高。

每個選題建議的「切角」欄、必明寫所對準的恐懼／驅動（例：40 歲女性怕「鬆弛 = 沒自信」）。這是 Discovery 的方向責任、現場真人救不回來的那塊（肥料分工、見 direction-principles.md 心法 B）。
