...
> version: 1.5 | last_updated: 2026-06-07
> 本檔目的：釐清共用規則的**SSoT 職責**與**升版順序**，避免升一份忘了同步另一份。
> v1.5：`direction-principles.md` 接入下游消費者 — discovery/SKILL.md（選題第一性）+ generation/SKILL.md（§共用協議 + 強標題協議步驟 0），「誰引用它」欄補實。
...
| `direction-principles.md` | **方向心法**（選題與 hook 的第一性原則：觀眾視角 + 肥料分工）| 選題 / hook 推理的上游心法層 | discovery/SKILL.md（選題第一性）+ generation/SKILL.md（§共用協議 + 強標題協議）；hook-templates.md / title-rules.md 上游對齊 |
